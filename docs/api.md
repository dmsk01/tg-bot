...
**Хранение файлов:**
- Файлы сохраняются локально в папке `uploads/` (создается автоматически при старте сервера)
- Это **временное хранилище** — файлы автоматически удаляются после завершения генерации (успешной или с ошибкой)
- Файлы не накапливаются на сервере
